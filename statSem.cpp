...
int stack::push(string in,int lineNo,int charNo)
{
    if((itr+1)>STACK_SIZE)
        return -1;
    this->itr++;
...
void stack::printf()
{
    ...
    {
        cout << arr[i].id<<"\t"<<arr[i].line<<":" << arr[i].charNo << endl;
    }
 }
    ...
    if(strcmp(root->fname,"<vars>")==0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    printf("Redefinition of Local variable %s at %d,%d\n",root->tokenInstance[0],root->lineNo[0],root->charNo[0]);
                    status = -1;
                }
            }
        }
